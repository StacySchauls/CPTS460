...
int meread(int fd, char *buf, int nbytes) // fd is dummy; RW from fosset
{
...
************************************************************/
  ...
  // printf("\n");
   ...
   pgtable = (u32 *)(p->pgdir[2048] & 0xFFFFFC00); // get VA of pgtable
   ...
   for (i=1, ph=aph; i <= phnum; ph++, i++){
       ...
       //printf("ph->offset=%x vaddr=%x memsize=%d\n", ph->offset, 
       //	      ph->vaddr, ph->memsize);
       //printf("Sec%d: ", i);
       ...
       pn = PA(ph->vaddr)/0x1000; // convert vaddr to page number
       //printf("page=%d ", pn);
       count = 0; 
       // addr = (char *)p->paddr;  // use PROC.paddr
       //printf("loading addr=%x\n", addr);

       addr = (char *)(pgtable[pn] & 0xFFFF000);
       //printf("loading addr=%x ", addr);
 ...
 *************************************************************/
	 ...
	 //       for (i=0; i<4; i++){
             ...
             meread(fd, dbuf, BLKSIZE);    // read by 4KB BLKSIZE
             //copy(addr, buf);
