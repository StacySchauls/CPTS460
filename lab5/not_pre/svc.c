...
int do_wait(){
  int status;
  kwait(&status);
  if (status < 0) printf("no child\n");
  else printf("zombie child %d ", status);
}
...
int svc_handler(int a, int b, int c, int d)
{
  ...
  switch(a){
    ...
    case 12: r = kwait(); break;
    case 14 : r = ksleep(b); break;
    case 13: r = kwakeup(b); break;
    case 11: r = kexit(b); break;
    case 16: r = exec(b); break;
    case 15: r =  fork(); break;//kfork(b); break;  //printf("returned\n"); break;//kfork("u1");
