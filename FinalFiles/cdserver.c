...
int ls(),cdchdir(),cdpwd(),cat(), ptable(), cdcp(),list_dir(),do_cd();
int rpwd();
int strcasecmp(), do_cmd();
...
int keatpath(filename) char *filename;
{
  char *cp;
...
int path_table()
{
   ...
   if (sVD){ // if has supplement VD, print its path table also
     ...
     spd = (struct iso_supplementary_descriptor *)cdbuf;
...
int main()
{
   ...
   *cwd  = *dirp;
   ...
   while(1){
     ...
     printf("CDSERVER: received request message=[%s] from %d\n", msg, client);
     strcpy(line, msg);
...
int copy64(from, to) char *from, *to;
{
  int i;
  for (i=0; i<64; i++){
    *to = *from;
